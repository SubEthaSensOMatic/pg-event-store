...
module.exports = async(pgp, database, schema, event, expectedAggregateVersion) => {
  ...
  const row = await database.oneOrNone(stmt, {
    ...
    expectedVersion: parseInt(expectedAggregateVersion, 10),
  });

  if (row == null) {
    throw new Error('This object has been modified or deleted by another user. Please review their changes before trying again.');
  } else {
    return {
